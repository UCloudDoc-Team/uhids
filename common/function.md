...
通过对黑客攻击时产生的操作行为进行分析捕捉，UHIDS能够检测到异常的、恶意的操作行为，一旦发现恶意的操作行为系统将触发告警，提醒用户处置潜在的安全风险。
...
为了更好的推动告警、安全事件的处置，UHIDS主机入侵检测支持通过接入模型服务平台（UModelVerse），这里引入了大模型来辅助用户针对指定的安全事件进行分析并提供处置的具体步骤措施，还能够在发生入侵事件的时候针对告警分析黑客入侵的渠道，找到入侵源修复漏洞风险突破口，彻底堵死黑客入侵利用入口解决安全问题！

**安全漏洞智能分析**

UHIDS主机入侵检测支持针主机产生的安全漏洞、风险进行智能分析，通过指定的大模型分析漏洞的处置方法，提高用户漏洞的处置效率，减少漏洞产生的风险情况。

**入侵事件智能分析**

支持通过指定大模型针对产生的入侵事件进行智能分析，通过大模型的分析、推理给用户输出入侵的源头以及产生的影响，最终提供整体的解决方案，帮助用户快速解决入侵问题。
